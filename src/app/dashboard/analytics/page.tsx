...
import { Bar, BarChart, CartesianGrid, XAxis, Line, LineChart, YAxis, Tooltip, Pie, PieChart, Cell } from "recharts"
...
function AnalyticsContent() {
  ...
  useEffect(() => {
    ...
    if (isCro && user.faculties && user.faculties.length > 0) {
      ...
      projectsQuery = query(
        projectsCollection,
        where("institute", "in", ["Parul Institute of Technology", "Parul Institute of Technology-Diploma studies"]),
      )
    ...
    const unsubscribe = onSnapshot(
      projectsQuery,
      (snapshot) => {
        const projectList: Project[] = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as Project)
        ...
        if (isPrincipal && user.institute && snapshot.empty) {
          ...
          onSnapshot(allProjectsQuery, (allSnapshot) => {
            ...
            const filteredList = allProjects.filter(
              (p) => p.institute && p.institute.toLowerCase() === user!.institute!.toLowerCase(),
            )
  ...
  useEffect(() => {
    if (filteredProjects.length > 0) {
      const years = new Set(filteredProjects.map((p) => getYear(parseISO(p.submissionDate))))
      setAvailableYears(
        Array.from(years)
          .sort((a, b) => b - a)
          .map(String),
  ...
  const submissionsData = useMemo(() => {
    ...
    const year = Number.parseInt(timeRange, 10)
  ...
  const { aggregationKey, aggregationLabel } = useMemo(() => {
    ...
    if (
      ...
      user?.email === "pit@paruluniversity.ac.in"
    ) {
  ...
  const getPageTitle = () => {
    ...
    if (user?.designation === "HOD" && user.department && user.institute)
      return `Analytics for ${user.department}, ${user.institute}`
    return "Analytics"
  }

  const getPageDescription = () => {
    if (user?.designation === "Principal" && !user.institute)
      return "Your institute information is not configured. Please update your profile to see institute-specific analytics."
    if (user?.role === "CRO" || user?.designation === "Principal" || user?.designation === "HOD")
      return "Visualize project data and submission trends for your scope."
