...
type SortableKeys = keyof Pick<User, "name" | "email" | "role" | "faculty" | "campus"> | "claimsCount"
...
function ProfileDetailsDialog({
  ...
  open,
  onOpenChange,
}: { user: User | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  if (!user) return null
...
export default function ManageUsersPage() {
  ...
  const handleRoleChange = useCallback(
    async (uid: string, newRole: User["role"], extraData?: Record<string, any>) => {
      try {
              ...
              <TableBody>
                {sortedAndFilteredUsers.map((user) => {
                  ...
                  const isActionsDisabled =
                    isCurrentUserLoggedIn || (isPrimarySuperAdmin && currentUser?.email !== PRIMARY_SUPER_ADMIN_EMAIL)
                  ...
                  return (
                    <TableRow key={user.uid}>
                      ...
                      <TableCell className="hidden sm:table-cell">{user.campus || "N/A"}</TableCell>
