...
export async function findUserByMisId(misId: string): Promise<{
  success: boolean
  user?: FoundUser
  staff?: { name: string; email: string; misId: string; campus: string; }
  users?: FoundUser[]
  error?: string
}> {
  try {
    ...
    if (foundUsers.length > 0) {
        if(foundUsers.length === 1) {
            const singleUser = foundUsers[0];
            return { success: true, user: singleUser.uid ? singleUser : undefined, staff: !singleUser.uid ? singleUser : undefined };
        }
...
export async function signAndUploadEndorsement(
  interestId: string,
