...
export async function findUserByMisId(misId: string): Promise<{
  ...
  try {
    ...
    return {
      success: false,
      error: "No user found with this MIS ID across any campus. Please check the MIS ID or ask them to sign up first.",
    }
  } catch (error: any) {
...
export async function notifySuperAdminsOnNewUser(userName: string, role: string) {
  try {
    ...
    await logActivity("ERROR", "Failed to notify super-admins about new user", {
      userName,
      role,
      error: error.message,
      stack: error.stack,
