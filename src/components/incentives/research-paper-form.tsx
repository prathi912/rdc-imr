        ...
        <form onSubmit={form.handleSubmit(() => handleSave('Pending'))}>
          <CardContent className="space-y-6 pt-6">
            ...
            <div className="rounded-lg border p-4 space-y-4 animate-in fade-in-0">
                ...
                <FormField control={form.control} name="relevantLink" render={({ field }) => ( <FormItem><FormLabel>DOI Link</FormLabel><div className="flex items-center gap-2"><FormControl><Input placeholder="https://doi.org/..." {...field} disabled={isSubmitting} /></FormControl><Button type="button" variant="outline" size="sm" onClick={handleFetchScopusData} disabled={isSubmitting || isFetchingScopus || !relevantLink || (indexType !== 'scopus' && indexType !== 'both')} title="Fetch data from Scopus">{isFetchingScopus ? <Loader2 className="h-4 w-4 animate-spin" /> : <Bot className="h-4 w-4" />}Scopus</Button><Button type="button" variant="outline" size="sm" onClick={handleFetchWosData} disabled={isSubmitting || isFetchingWos || !relevantLink || (indexType !== 'wos' && indexType !== 'both')} title="Fetch data from Web of Science">{isFetchingWos ? <Loader2 className="h-4 w-4 animate-spin" /> : <Bot className="h-4 w-4" />}WoS</Button></div><FormMessage /></FormItem> )}/>
                {(indexType === 'scopus' || indexType === 'both') && (
                    <FormField control={form.control} name="scopusLink" render={({ field }) => (
                        <FormItem>
                            <FormLabel>Scopus Link</FormLabel>
                            <FormControl><Input placeholder="https://www.scopus.com/record/..." {...field} disabled={isSubmitting} /></FormControl>
                            <FormMessage />
                        </FormItem>
                    )}
                )}
                ...
                <FormField control={form.control} name="paperTitle" render={({ field }) => ( <FormItem><FormLabel>Title of the Paper published</FormLabel><FormControl><Textarea placeholder="Enter the full title of your paper" {...field} disabled={isSubmitting} /></FormControl><FormMessage /></FormItem> )}/>
