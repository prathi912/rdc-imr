...
const conferenceSchema = z
  .object({
    ...
    participationCertificate: z.any().refine((files) => files?.length > 0, 'Participation certificate is required.').refine(files => files?.[0]?.type === 'application/pdf', 'File must be a PDF.').refine(files => files?.[0]?.size <= MAX_FILE_SIZE, 'File must be less than 10 MB.'),
            ...
            <div className="rounded-lg border p-4 space-y-6 animate-in fade-in-0">
                <div>
                    ...
                    <div className="space-y-4 mt-4">
                        ...
                        <FormField name="abstractUpload" control={form.control} render={({ field: { value, onChange, ...fieldProps } }) => ( <FormItem><FormLabel>Attach Full Abstract (PDF, Below 10MB)</FormLabel><FormControl><Input {...fieldProps} type="file" onChange={(e) => onChange(e.target.files)} disabled={isFormDisabled} accept="application/pdf" /></FormControl><FormMessage /></FormItem> )} />
                        <FormField name="participationCertificate" control={form.control} render={({ field: { value, onChange, ...fieldProps } }) => ( <FormItem><FormLabel>Attach Participation/Presentation Certificate (PDF, Below 10MB)</FormLabel><FormControl><Input {...fieldProps} type="file" onChange={(e) => onChange(e.target.files)} disabled={isFormDisabled} accept="application/pdf" /></FormControl><FormMessage /></FormItem> )} />
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
