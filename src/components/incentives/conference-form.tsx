...
const conferenceSchema = z
  ...
  .refine(data => !(data.conferenceMode === 'Offline' && data.travelMode === 'Air') || (!!data.flightTickets && data.flightTickets.length > 0), { message: 'Flight tickets are required for air travel.', path: ['flightTickets']})
  ...
  async function onSubmit(data: ConferenceFormValues) {
    ...
    try {
      ...
      const claimData: Omit<IncentiveClaim, 'id'> = {
        ...
        flightTicketsUrl: await uploadFileHelper(data.flightTickets?.[0], 'conference-flight-tickets'),
        uid: user.uid,
        userName: user.name,
