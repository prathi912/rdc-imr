  ...
  useEffect(() => {
    ...
    async function getProjects() {
      ...
      try {
        ...
        // Filter for projects assigned to the current evaluator that they haven't reviewed yet
        const pendingForCurrentUser = projectList.filter(p => 
            p.meetingDetails?.assignedEvaluators?.includes(user.uid) &&
          ...
          <ProjectList projects={filteredProjects} userRole="Evaluator" />
           ...
           <Card>
            <CardContent className="pt-6 text-center text-muted-foreground">
              There are no projects currently assigned to you for evaluation.
            </CardContent>
          </Card>
        )}
      </div>
